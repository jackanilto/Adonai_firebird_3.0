...
procedure TfrmCadIgreja.BtnSairClick(Sender: TObject);
  begin
    if Application.MessageBox('Deseja fechar o formulario?','Confirmação',
...
begin
  ...
  begin
    try
      ...
      // Certifique-se de que os campos estão presentes antes de acessá-los
      if Assigned(DM.QueryIgrejas.FieldByName('NOME_IGREJA')) then
        DM.QueryIgrejas.FieldByName('NOME_IGREJA').AsString := DBEditIGREJA.Text;

      // Adicione verificações semelhantes para outros campos
